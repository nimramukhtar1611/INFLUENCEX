...
const Contracts = () => {
  ...
  const actionButton = user?.userType === 'brand' ? (
    <Link to="/brand/createcontract">
     <button className="
...
">
  ...
  return (
    <div >
      <BrandLayout
        title="Contracts"
        subtitle={user?.userType === 'brand' 
          ? "Manage and track all your legal agreements and partnerships." 
          : "View and manage contracts sent by brands for your signature."}
  ...
  {contracts.length > 0 ? (
    <div className="space-y-4">
      ...
      {contracts.map((contract) => (
        <div 
          key={contract._id} 
          onClick={() => navigate(`/brand/contracts/${contract._id}`)}
          className={`
          ...
          <div className="col-span-3 mt-4 md:mt-0 flex flex-col items-center gap-3 px-4">
            ...
            <div className="text-center">
              <div className={`text-[10px] font-bold tracking-tight ${isDark ? 'text-zinc-400' : 'text-zinc-600'}`}>
                {contract.creatorId?.displayName || 'Unknown Creator'}
              </div>
    ...
    `}>
      ...
      <h3 className="text-lg font-bold tracking-tight">No Active Contracts</h3>
      <p className="text-[12px] text-zinc-500 mt-2 max-w-[260px] text-center leading-relaxed">
        Create new agreements to formalize partnerships and track legal obligations.
      </p>
    </div>
  )}
</div>
