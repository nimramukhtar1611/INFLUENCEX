...
const MessageBubble = ({ message, isOwn, onReaction, onDelete, onReply, isDark }) => {
    ...
    >
      {!isOwn && (
        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mr-2 mt-1 ${
          isDark ? 'bg-zinc-900/50' : 'bg-gray-800'
        }`}>
      ...
      <div className={`max-w-[85%] md:max-w-[70%] ${isOwn ? 'mr-2' : 'ml-2'}`}>
        ...
        <div className="relative">
          {showActions && !message.isDeleted && (
            <div className={`absolute ${isOwn ? 'left-0 -translate-x-full' : 'right-0 translate-x-full'} top-0 flex items-center gap-1 ${isDark ? 'bg-zinc-900/50' : 'bg-white'} rounded-lg shadow-lg border border-gray-200 p-1 z-10`}>
              <button onClick={() => setShowReactions(!showReactions)} className={`p-1.5 rounded-lg ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}><Smile className={`w-4 h-4 ${isDark ? 'text-gray-300' : 'text-gray-700'}`} /></button>
              <button onClick={() => onReply(message)} className={`p-1.5 rounded-lg ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}><Reply className={`w-4 h-4 ${isDark ? 'text-gray-300' : 'text-gray-700'}`} /></button>
              <button onClick={() => navigator.clipboard.writeText(message.content)} className={`p-1.5 rounded-lg ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}><Copy className={`w-4 h-4 ${isDark ? 'text-gray-300' : 'text-gray-700'}`} /></button>
              {isOwn && (
                <button onClick={() => onDelete(message._id)} className={`p-1.5 rounded-lg ${isDark ? 'hover:bg-red-900/20' : 'hover:bg-red-100'}`}><Trash2 className="w-4 h-4 text-red-600" /></button>
              )}
            </div>
          )}
              ...
              <div className="flex gap-1">
                {reactions.map(r => (
                  <button key={r} onClick={() => { onReaction(message._id, r); setShowReactions(false); }}
                    className={`w-8 h-8 rounded-lg text-xl hover:scale-110 transition-all ${isDark ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}>{r}</button>
                ))}
              </div>
            </div>
          )}
          ...
          <div className={`rounded-2xl p-3 ${
            message.isDeleted
              ? (isDark ? 'bg-zinc-900/50 text-gray-400 italic' : 'bg-gray-100 text-gray-600 italic')
              : message.contentType === 'deal_offer'
              ? 'bg-gradient-to-r from-green-500 to-teal-500 text-white'
              : isOwn
                ? (isDark ? 'bg-zinc-900/50 text-white border border-gray-700' : 'bg-gray-800 text-white')
                : (isDark ? 'bg-zinc-900/50 text-gray-100 border border-gray-700' : 'bg-white text-gray-900 shadow-sm border border-gray-200')
          }`}>
            {!message.isDeleted && message.replyTo && (
              <div className={`mb-2 p-2 rounded-lg text-sm ${isOwn ? 'bg-indigo-700' : (isDark ? 'bg-zinc-900/50' : 'bg-gray-100')}`}>
...
const CreatorInbox = () => {
  ...
  }, []); // Remove loadConversations dependency to prevent infinite calls
  ...
  return (
    <div >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-3xl font-light tracking-tight font-semibold">Creator <span className="font-bold">Inbox</span></h1>
          <p className={`text-sm mt-1 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>Manage conversations and collaborate with brands.</p>
        </div>
      </div>
...
                    <span className="ml-1 rounded-full w-4 h-4 inline-flex items-center justify-center text-xs bg-black text-white">
                      {totalUnread}
                    </span>
                  )}
                </button>
...
            </div> 
      ...
      {!selectedConversation ? (
      <div className={`flex flex-col h-full rounded-[2rem] border overflow-hidden transition-all duration-500 ${isDark ? 'bg-zinc-900/40 border-zinc-800 shadow-2xl' : 'bg-white border-zinc-100 shadow-sm'}`}>
  {/* Header Section */}
  <div className={`p-6 border-b flex items-center justify-between ${isDark ? 'border-zinc-800 bg-zinc-900/20' : 'border-zinc-50 bg-zinc-50/50'}`}>
    <h2 className={`text-[11px] font-black uppercase tracking-[0.25em] ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>
      Operational Comms
    </h2>
    <div className={`w-2 h-2 rounded-full animate-pulse ${isDark ? 'bg-emerald-500/50 shadow-[0_0_8px_#10b981]' : 'bg-emerald-500'}`} />
  </div>

  <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
    {loading ? (
      <div className="flex flex-col items-center justify-center py-20 opacity-50">
        <Loader className="w-6 h-6 animate-spin text-zinc-500 mb-4 stroke-[1.5px]" />
        ...
        {filteredConversations.map(conv => {
          ...
          return (
            <div
              ...
              className={`
                group relative p-4 cursor-pointer transition-all duration-300 rounded-[1.5rem]
                ${isSelected 
                  ? (isDark ? 'bg-zinc-800/80 shadow-inner' : 'bg-zinc-50 shadow-inner') 
                  : (isDark ? 'hover:bg-zinc-800/40' : 'hover:bg-zinc-50/50')}
              `}
            >
              ...
              <div className="flex items-start gap-4">
                {/* Avatar with Enhanced Signal Status */}
                <div className="relative shrink-0">
                  <div className={`
                    w-11 h-11 rounded-2xl overflow-hidden border-2 p-0.5 transition-transform duration-500 group-hover:scale-105
                  ...
                  {isUserOnline(conv) && (
                    <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-500 rounded-full border-[3px] border-zinc-900 shadow-[0_0_10px_#10b981]" />
                  )}
                </div>
                
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <h3 className={`font-bold text-[13px] tracking-tight truncate ${isDark ? 'text-zinc-100' : 'text-zinc-900'}`}>
                      {conv.deal_id?.campaignId?.title || conv.campaign_id?.title || getConversationName(conv)}
                    </h3>
                    {conv.lastMessageAt && (
                      <span className={`text-[9px] font-mono font-bold ml-2 ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}>
                  ...
                  <div className="flex items-center gap-1.5 mb-2">
                    <span className="text-[9px] font-black uppercase tracking-widest text-indigo-500/80">
                      ID: {getConversationName(conv)}
                    </span>
                  </div>

                  <p className={`text-[11px] font-medium truncate mb-2 ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>
                    <span className={isDark ? 'text-zinc-300' : 'text-zinc-600'}>
                      {conv.lastMessage?.senderId?._id === user?._id ? 'Local: ' : 'Remote: '}
                    </span>
                    {conv.lastMessage?.contentType === 'deal_offer'
                      ? '💰 PRIORITY OFFER'
                      : conv.lastMessage?.content || 'NO LOG DATA'}
                  </p>

                  {conv.deal_id && (
                    <div className="flex items-center gap-2">
                      <span className={`
                        inline-flex items-center px-2.5 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-tighter
                        ${getDealStatusDisplay(conv.deal_id.status).statusClass}
                        border border-current/10
                      `}>
                        <span className="w-1 h-1 rounded-full bg-current mr-1.5 animate-pulse" />
                        {formatCurrency(conv.deal_id.budget)} · {conv.deal_id.status}
                      </span>
                    </div>
                  )}
                </div>
      ...
      <div className="flex flex-col items-center justify-center py-20 px-6 opacity-30">
        <MessageSquare className="w-10 h-10 mb-4 stroke-[1px]" />
        <h3 className="text-[10px] font-black uppercase tracking-[0.2em]">Silence Detected</h3>
        <p className="text-[9px] font-bold mt-1 text-center">Awaiting incoming transmissions</p>
      </div>
    )}
  </div>
</div>
...
>
  ...
  <span className="hidden md:inline">View Deal</span>
  <span className="md:hidden">Deal</span>
            ...
            {selectedConversation.deal_id && (
              <div className={`px-3 md:px-4 py-2  flex flex-col md:flex-row md:items-center justify-between gap-2 ${
                isDark 
                  ? 'bg-zinc-900/50'
                ...
                <Link
                  to={`/creator/deals/${selectedConversation.deal_id._id}`}
      ...
      <Modal isOpen={showDealModal} onClose={() => setShowDealModal(false)} title="Deal Details">
        {dealDetails && (
          <div className="space-y-4">
            ...
            <Link
              to={`/creator/deals/${dealDetails._id}`}
