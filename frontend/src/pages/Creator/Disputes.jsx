...
const Disputes = () => {
  ...
  const fetchUserDeals = async () => {
    ...
    try {
      const res = await dealService.getCreatorDeals('all', 1, 50);
      if (res?.success) {
  ...
  return (
    <div className={`max-w-6xl mx-auto p-6 space-y-8 ${isDark ? 'text-zinc-100' : 'text-zinc-900'}`}>
      ...
      <div className="flex items-center justify-between">
        <button 
          onClick={() => navigate(-1)} 
          className="group flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-zinc-500 hover:text-zinc-300 transition-all duration-300"
        >
        ...
        <div className="text-right">
          ...
          <p className="text-sm text-zinc-500">Manage and resolve disputes with brands</p>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`
                group relative px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider 
                transition-all duration-300 transform hover:scale-105 active:scale-95
                border
                ${isDark 
                  ? 'bg-zinc-900 border-zinc-700 text-zinc-300 hover:bg-zinc-800 hover:border-zinc-600 hover:text-white hover:shadow-lg hover:shadow-zinc-500/10' 
                  : 'bg-white border-zinc-300 text-zinc-700 hover:bg-zinc-50 hover:border-zinc-400 hover:text-zinc-900 hover:shadow-lg hover:shadow-zinc-200/20'}
              `}
            >
                  ...
                  <div className="col-span-4 flex items-center gap-5">
                    ...
                    <div className="flex flex-col min-w-0">
                      ...
                      <span className={`text-[11px] font-medium mt-1 uppercase tracking-wider ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>
                        Deal: {dispute.deal_id?._id?.slice(-6) || 'N/A'} • {dispute.deal_id?.brandId?.brandName || dispute.deal_id?.brandId?.fullName || 'Brand'} • ${formatCurrency(dispute.deal_id?.budget || 0)}
                      </span>
        ...
        <div className="space-y-4">
            ...
            >
              ...
              {userDeals.map(deal => (
                <option key={deal._id} className="!bg-black text-white" value={deal._id}>
                  Deal: {deal._id?.slice(-6) || 'N/A'} - {deal.brandId?.brandName || deal.brandId?.fullName || 'Brand'} - ${formatCurrency(deal.budget || 0)} (Campaign: {deal.campaignId?.title || 'N/A'})
                </option>
              ))}
            </select>
          </div>
          ...
          <div className={`p-4 rounded-lg ${
            ...
            <p className={`text-sm ${
              ...
              <strong>Note:</strong> The brand will be notified and an admin will review your dispute within 24 hours.
            </p>
          </div>
        </div>
  ...
  variant='secondary' onClick={handleCreateDispute}>
            Submit Dispute
          </Button>
        </div>
      </Modal>
        ...
        {selectedDispute && (
          <div className={`space-y-6 ${isDark ? 'text-zinc-200' : 'text-zinc-800'}`}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold leading-none mb-1">
                    {selectedDispute.title || 'Your Dispute Case'}
                  </h2>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono text-zinc-500">#{selectedDispute._id?.slice(-8) || selectedDispute.dispute_id}</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {[
                { label: 'Campaign', val: selectedDispute.campaignId?.title || selectedDispute.campaign?.title || 'N/A', icon: Target },
                { label: 'Dispute Type', val: (selectedDispute.type || 'unknown').replace(/_/g, ' '), icon: FileText },
                { label: 'Priority', val: selectedDispute.priority || 'normal', icon: AlertCircle },
                { label: 'Amount', val: selectedDispute.dealId?.budget ? formatCurrency(selectedDispute.dealId.budget) : 'N/A', icon: DollarSign }
              ].map((item, i) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="space-y-2">
                    <div className={`p-3 rounded-lg border ${isDark ? 'bg-zinc-900/20 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                      ...
                      <p className="text-xs font-medium">
                        {selectedDispute.raisedBy?.userId?.fullName || selectedDispute.raisedBy?.fullName || selectedDispute.raisedBy?.displayName || user?.fullName || user?.displayName || 'You'}
                      </p>
                    </div>
                    <div className={`p-3 rounded-lg border ${isDark ? 'bg-zinc-900/20 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                      <div className="flex items-center gap-2 mb-1">
                      ...
                      <p className="text-xs font-medium">
                        {selectedDispute.against?.userId?.fullName || selectedDispute.against?.fullName || selectedDispute.against?.displayName || selectedDispute.brandId?.displayName || selectedDispute.brand?.fullName || 'Brand Partner'}
                      </p>
                    </div>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div className="space-y-3 max-h-60 overflow-y-auto overflow-x-hidden scrollbar-hover">
                      ...
                      [...selectedDispute.messages].reverse().map((msg) => (
                        <div key={msg._id} className="text-xs border-l-2 border-zinc-800 pl-3 py-1">
                          <div className="flex justify-between mb-1 opacity-70">
                            <span className="font-bold break-words">{msg.sender_id?.fullName || msg.sender_id?.displayName || msg.sender_id?.name || msg.sender?.fullName || msg.sender?.displayName || msg.sender?.name || msg.from?.fullName || msg.from?.displayName || msg.from?.name || 'Unknown User'}</span>
                            <span>{timeAgo(msg.created_at)}</span>
                          </div>
                          <p className="text-zinc-400 break-words">{msg.message || msg.content || 'No message content'}</p>
                        </div>
                      ))
                    ) : (
                ...
                <div className={`p-3 rounded-xl border ${isDark ? 'bg-zinc-900/80 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
                  <div className="space-y-3">
                    <div className="flex justify-between text-[11px]">
                      ...
                      <span className={`font-bold uppercase ${selectedDispute.priority === 'high' ? 'text-red-500' : 'text-zinc-400'}`}>{selectedDispute.priority || 'normal'}</span>
                    </div>
                    <div className="flex justify-between text-[11px]">
